...
# internal modules
from color_brains.data.color_loaders.loaders import CmapLoader, InvalidShapeError
...
class TestLoaders(TestCase):
    def test_empty_colors(self):
        with self.assertRaises(InvalidShapeError):
            CmapLoader.make_dataframe(test_colors_empty)

    def test_one_colors(self):
        df = CmapLoader.make_dataframe(test_colors_one)
        assert df.shape == (1, 3)

    def test_many_colors(self):
        df = CmapLoader.make_dataframe(test_colors_many)
        assert df.shape == (5, 3)
    
    def test_has_alpha(self):
        df = CmapLoader.make_dataframe(test_has_alpha)
        assert df.shape == (2, 3)
    
    def test_convert_integers(self):
        df = CmapLoader.make_dataframe(test_colors_0_255)
